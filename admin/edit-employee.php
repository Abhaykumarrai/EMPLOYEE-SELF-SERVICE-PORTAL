...
if (strlen($_SESSION['etmsaid']==0)) {
 ...
 $dateofretir=$_POST['dateofretir'];
 $leaveapp=$_POST['leaveapp'];
 $leavcontroling=$_POST['leavcontroling'];
 $leaverepor=$_POST['leaverepor'];
 $desc=$_POST['desc'];
 $eid=$_GET['editid'];
$sql="update tblemployee set DepartmentID=:deptname,EmpType=:emptype,EmpId=:empid,EmpName=:empname,EmpEmail=:empemail,EmpContactNumber=:empcontno,Designation=:designation,EmpDateofbirth=:empdob,EmpAddress=:empadd,EmpDateofjoining=:empdoj,ReportingOfficerName=:reportingofficername,DateOfrRtirement=:dateofretir,LeaveApproving=:leaveapp,LeaveReporting=:leaverepor,LeaveControling=:leavcontroling,Description=:desc where ID=:eid";
...
$query->bindParam(':dateofretir',$dateofretir,PDO::PARAM_STR);
$query->bindParam(':leaveapp',$leaveapp,PDO::PARAM_STR);
$query->bindParam(':leavcontroling',$leavcontroling,PDO::PARAM_STR);
$query->bindParam(':leaverepor',$leaverepor,PDO::PARAM_STR);
...
<html lang="en">
   <head>
      <title>fssai Employee Self service|| Update Employee</title>
                                          ...
                                          <div class="padding_infor_info">
                                             <div class="alert alert-primary" role="alert">
                                                <form method="post">
                                                   <?php
$eid=$_GET['editid'];
$sql="SELECT tbldepartment.ID as did, tbldepartment.DepartmentName,tblemployee.ID as eid,tblemployee.DepartmentID,tblemployee.EmpType,tblemployee.EmpName,tblemployee.EmpId,tblemployee.EmpEmail,tblemployee.EmpContactNumber,tblemployee.EmpDateofjoining,tblemployee.ReportingOfficerName,tblemployee.Designation,tblemployee.EmpDateofbirth,tblemployee.EmpAddress,tblemployee.DateOfrRtirement,tblemployee.LeaveApproving,tblemployee.LeaveReporting,tblemployee.LeaveControling,tblemployee.Description,tblemployee.ProfilePic,tblemployee.CreationDate from tblemployee join tbldepartment on tbldepartment.ID=tblemployee.DepartmentID where tblemployee.ID=:eid";
$query = $dbh -> prepare($sql);
$query->bindParam(':eid',$eid,PDO::PARAM_STR);
$query->execute();
...
<option value="<?php echo htmlentities($row2->ID);?>"><?php echo htmlentities($row2->DepartmentName
 ...
 <?php } ?>
                           ...
                           <div class="field">
                              ...
                              <input type="text" name="emptype" value="<?php echo ($row->EmpType);?>" class="form-control" required='true'>
                           </div>
                          ...
                          <br>
                           ...
                           <br>
                           <div class="field">
                              <label class="label_field">Reporting Officer</label>
                              <input type="text" name="reportingofficername" value="<?php echo htmlentities($row->ReportingOfficerName);?>" class="form-control" required='true'>
                           </div>

                           <br>
                           <div class="field">
                              <label class="label_field">Date of Retirement/Deputation</label>
                              <input type="text" name="dateofretir" value="<?php echo htmlentities($row->DateOfrRtirement);?>" class="form-control" required='true'>
                           </div>

                           <br>
                           <div class="field">
                              <label class="label_field">Leave Approving Authority</label>
                              <input type="text" name="leaveapp" value="<?php echo htmlentities($row->LeaveApproving);?>" class="form-control" required='true'>
                           </div>

                           <br>
                           <div class="field">
                              <label class="label_field">Leave Controling</label>
                              <input type="text" name="leavcontroling" value="<?php echo htmlentities($row->LeaveControling);?>" class="form-control" required='true'>
                           </div>

                           <br>
                           <div class="field">
                              <label class="label_field">Leave Reporting Officer Name</label>
                              <input type="text" name="leaverepor" value="<?php echo htmlentities($row->LeaveReporting);?>" class="form-control" required='true'>
                           </div>
