...
if (strlen($_SESSION['etmsaid']==0)) {
 ...
 $sdate=$_POST['sdate'];
 $tedate=$_POST['tedate'];
$sql="insert into tbleoffice(DeptID,AssignTo,E_Office,StartDate,EndDate)values(:deptid,:emplist,:eoffice,:sdate,:tedate)";
$query=$dbh->prepare($sql);
$query->bindParam(':deptid',$deptid,PDO::PARAM_STR);
$query->bindParam(':emplist',$emplist,PDO::PARAM_STR);
$query->bindParam(':eoffice',$eoffice,PDO::PARAM_STR);
...
$query->bindParam(':tedate',$tedate,PDO::PARAM_STR);
 ...
 $query->execute();
   ...
   if ($LastInsertId>0) {
    echo '<script>alert("E-Office has been assigned to the employee.")</script>';
echo "<script>window.location.href ='add-eoffice.php'</script>";
  }
  else
    {
...
<option value="<?php echo htmlentities($row2->ID);?>"><?php echo htmlentities($row2->DepartmentName
 ...
 <?php } ?>
                           ...
                           <br>
                           <div class="field">
                              <label class="label_field">E-Office</label>
                              <input type="text" name="eoffice" value="" class="form-control" required='true'>
                           </div>
