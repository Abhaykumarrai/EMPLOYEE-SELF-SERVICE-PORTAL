...
if (strlen($_SESSION['etmsaid']==0)) {
 ...
 $dscnumber=$_POST['dscnumber'];
 $sdate=$_POST['sdate'];
 $tedate=$_POST['tedate'];
$sql="insert into tbldscno(DeptID,Assignto,DSCNO,StartDate,EndDate)values(:deptid,:emplist,:dscnumber,:sdate,:tedate)";
$query=$dbh->prepare($sql);
$query->bindParam(':deptid',$deptid,PDO::PARAM_STR);
$query->bindParam(':emplist',$emplist,PDO::PARAM_STR);
$query->bindParam(':dscnumber',$dscnumber,PDO::PARAM_STR);
...
$query->bindParam(':tedate',$tedate,PDO::PARAM_STR);
 ...
 $query->execute();
   ...
   if ($LastInsertId>0) {
    echo '<script>alert("DSC number has been assigned to the employee.")</script>';
echo "<script>window.location.href ='add-dscno.php'</script>";
  }
  else
    {
...
<html lang="en">
   <head>
      <title>fssai Employee self service || Add DSC</title>
...
<option value="<?php echo htmlentities($row2->ID);?>"><?php echo htmlentities($row2->DepartmentName
 ...
 <?php } ?>
                           ...
                           <br>
                           <div class="field">
                              <label class="label_field">DSC NUMBER</label>
                              <input type="text" name="dscnumber" value="" class="form-control" required='true'>
                           </div>
